...
async function fetchSkuMappingRowForTable(
  ...
  const [row] = await db
    .select({
      ...
      hasAutoGeneratedDraft: sql<boolean>`coalesce((
        exists (
          select 1 from product_bom_draft d
          where d.parent_sku = ${sku_mappings.global_sku}
            and d.source in ('heuristic', 'sketchup_geometry')
        )
        or exists (
          select 1 from recipe_estimates_draft e
          where e.root_sku = ${sku_mappings.global_sku}
        )
      ), false)`.mapWith(Boolean),
    })
    .from(sku_mappings)
    .leftJoin(
      finished_goods_catalog,
  ...
  const { mapping, catalog, bomCount, hasAutoGeneratedDraft } = row;
  return {
    ...
    hasAutoGeneratedDraft: Boolean(hasAutoGeneratedDraft),
    catalog: catalog
      ? {
          msrp: catalog.msrp,
          cost: catalog.cost,
