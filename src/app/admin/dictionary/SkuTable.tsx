...
const WORKFLOW_FILTERS: Array<{
...
}> = [
  {
    ...
    label: "🔥 Phase 1 SOW",
    activeClass:
      "border-amber-400/70 bg-gradient-to-r from-amber-500/25 to-orange-600/15 text-amber-100 shadow-[0_0_20px_-8px_rgba(251,191,36,0.55)]",
    idleClass:
      "border-amber-500/25 bg-zinc-950/80 text-amber-200/70 hover:border-amber-400/50 hover:text-amber-100",
  },
  {
    id: "phase2",
    label: "🚀 Phase 2 SOW",
    activeClass:
      "border-sky-400/70 bg-gradient-to-r from-sky-500/25 to-emerald-600/15 text-sky-100 shadow-[0_0_20px_-8px_rgba(56,189,248,0.45)]",
    idleClass:
      "border-sky-500/25 bg-zinc-950/80 text-sky-200/70 hover:border-sky-400/50 hover:text-sky-100",
  },
  {
    id: "auto3d",
    label: "🤖 3D Auto-Generated",
    activeClass:
      "border-emerald-400/70 bg-gradient-to-r from-emerald-500/25 to-teal-600/15 text-emerald-100 shadow-[0_0_20px_-8px_rgba(52,211,153,0.5)]",
    idleClass:
      "border-emerald-500/25 bg-zinc-950/80 text-emerald-200/70 hover:border-emerald-400/50 hover:text-emerald-100",
  },
];
...
export function SkuTable({ rows: initialRows, operatorEmail }: SkuTableProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (workflowFilters.size > 0) {
      byTab = byTab.filter((row) =>
        rowMatchesWorkflowFilters(row, workflowFilters),
      );
    }
  ...
  }, [selectedCategories, workflowFilters, query, rows, quickFilter]);
  ...
  const workflowCounts = useMemo(() => {
    ...
    let auto3d = 0;
    for (const row of rows) {
      if (rowMatchesWorkflowFilters(row, new Set(["phase1"]))) phase1 += 1;
      if (rowMatchesWorkflowFilters(row, new Set(["phase2"]))) phase2 += 1;
      if (row.hasAutoGeneratedDraft) auto3d += 1;
    }
    return { phase1, phase2, auto3d };
  }, [rows]);

  function toggleWorkflowFilter(id: SowWorkflowFilter): void {
  ...
  return (
    <section className="space-y-3 p-2 sm:p-3">
      ...
      <div
        ...
        data-tour="dictionary-workflow-filters"
      >
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <p className="text-[11px] font-medium uppercase tracking-wider text-zinc-400">
            Workflow filters{" "}
            <span className="font-normal normal-case tracking-normal text-zinc-600">
              (SOW deliverables · compose with search)
            </span>
          </p>
          {workflowFilters.size > 0 ? (
            <button
              type="button"
              onClick={() => setWorkflowFilters(new Set())}
        ...
        <div className="flex flex-wrap gap-2">
          {WORKFLOW_FILTERS.map((filter) => {
            const active = workflowFilters.has(filter.id);
            const count =
              filter.id === "phase1"
                ? workflowCounts.phase1
                : filter.id === "phase2"
                  ? workflowCounts.phase2
                  : workflowCounts.auto3d;
            return (
              <button
                key={filter.id}
                type="button"
                aria-pressed={active}
                data-testid={`dictionary-workflow-${filter.id}`}
                onClick={() => toggleWorkflowFilter(filter.id)}
              ...
              >
                ...
                <span className="rounded bg-black/25 px-1.5 py-0.5 font-mono text-[10px] tabular-nums opacity-90">
                  {count}
                </span>
              </button>
            );
        ...
        <p className="mt-2 text-[10px] text-zinc-600">
          Phase lists: {SOW_PHASE_1.length} P1 · {SOW_PHASE_2.length} P2 names
          (fuzzy match on catalog Name)
        </p>
      </div>
